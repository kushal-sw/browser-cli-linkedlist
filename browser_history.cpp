...
// ==================== ANSI Colors ====================
namespace C {
    const string R="\033[0m", RED="\033[1;31m", GRN="\033[1;32m", YEL="\033[1;33m";
    const string BLU="\033[1;34m", MAG="\033[1;35m", CYN="\033[1;36m", WHT="\033[1;37m";
...
class Stack {
    ...
    void removeBottom() {
        if (!top) return;
        if (!top->next) { delete top; top = nullptr; cnt--; return; }
        Node* c = top;
...
public:
    ...
    void push(const T& v) {
        if (maxSz > 0 && cnt >= maxSz) removeBottom();
        Node* n = new Node(v);
        n->next = top;
        top = n;
        cnt++;
    ...
    void clear() {
        while (top) { Node* t = top; top = top->next; delete t; cnt--; }
    }

    vector<T> toVector() const {
        vector<T> r;
        Node* c = top;
        while (c) { r.push_back(c->data); c = c->next; }
        return r;
    }
};
...
public:
    ...
    vector<HistoryEntry> deleteByURL(const string& url) {
        ...
        while (head && toLower(head->data.url) == tgt) {
            Node* t = head; del.push_back(t->data);
            head = head->next; delete t; cnt--;
        }
        if (!head) return del;
        Node* c = head;
        while (c->next) {
            if (toLower(c->next->data.url) == tgt) {
                Node* t = c->next; del.push_back(t->data);
                c->next = t->next; delete t; cnt--;
            } else c = c->next;
        }
        return del;
    }
    ...
    vector<int> search(const string& url) const {
        ...
        Node* c = head; int p = 1;
        while (c) {
            if (toLower(c->data.url) == tgt) pos.push_back(p);
            c = c->next; p++;
        }
        return pos;
    }
    ...
    bool contains(const string& url) const {
        Node* c = head;
        while (c) { if (toLower(c->data.url) == toLower(url)) return true; c = c->next; }
        return false;
    }
...
void insertionSortByCount(vector<HistoryEntry>& arr, unordered_map<string, int>& freq) {
    for (int i = 1; i < (int)arr.size(); i++) {
        HistoryEntry key = arr[i];
        int j = i - 1;
...
class BrowserHistoryManager {
    ...
    void printOK(const string& m) { cout << C::GRN << "  ✅ " << m << C::R << endl; }
    void printErr(const string& m) { cout << C::RED << "  ❌ " << m << C::R << endl; }
    void printInfo(const string& m) { cout << C::YEL << "  ℹ️  " << m << C::R << endl; }
    ...
    void displayNavBar() {
        ...
        if (currentPage.empty()) {
            cout << C::DIM << "🌐 Search or enter address..." << string(26, ' ') << C::R;
        } else {
            string star = bookmarks.contains(currentPage) ? C::YEL + " 🔖" + C::R : "   ";
            string urlStr = "🔒 https://" + currentPage;
            int pad = max(0, 40 - (int)urlStr.length());
            cout << C::GRN << "🔒 " << C::R << C::WHT << "https://" << currentPage << C::R << string(pad, ' ') << star;
        }
        cout << C::DIM << " │" << C::R << endl;
        cout << C::DIM << "  ├──────────────────────────────────────────────────────────┤" << C::R << endl;
        ...
        for (int i = 0; i < f_end; i++) cout << C::DIM << " → " << fwd[i] << C::R;
        if ((int)fwd.size() > 2) cout << C::DIM << " → ..." << C::R;
    ...
    void displayMenu() {
        cout << endl;
        cout << C::MAG << C::BOLD << "  ╭───────────────── BROWSER MENU ─────────────────╮" << C::R << endl;
        cout << C::WHT << "  │   " << C::CYN << "1." << C::WHT << " 🌐 Visit URL         " << C::CYN << "8." << C::WHT << " 🔖 Bookmark URL   │" << C::R << endl;
        cout << C::WHT << "  │   " << C::CYN << "2." << C::WHT << " ◀  Go Back           " << C::CYN << "9." << C::WHT << " 📂 View Bookmarks │" << C::R << endl;
        cout << C::WHT << "  │   " << C::CYN << "3." << C::WHT << " ▶  Go Forward       " << C::CYN << "10." << C::WHT << " 🗑️  Rm Bookmark    │" << C::R << endl;
        cout << C::WHT << "  │   " << C::CYN << "4." << C::WHT << " 🕒 History          " << C::CYN << "11." << C::WHT << " 📊 Most Visited   │" << C::R << endl;
        cout << C::WHT << "  │   " << C::CYN << "5." << C::WHT << " 🔍 Search           " << C::CYN << "12." << C::WHT << " 🗂️  Sort History   │" << C::R << endl;
        cout << C::WHT << "  │   " << C::CYN << "6." << C::WHT << " 🗑️  Delete URL       " << C::CYN << "13." << C::RED << " 🚪 Exit Session   │" << C::R << endl;
        cout << C::WHT << "  │   " << C::CYN << "7." << C::WHT << " ↩️  Undo Delete                         │" << C::R << endl;
        cout << C::MAG << C::BOLD << "  ╰────────────────────────────────────────────────╯" << C::R << endl;
        cout << C::YEL << "  ❯ Enter choice: " << C::R;
    }

    // Get next session ID by scanning saved-history folder
    int getNextSessionId() {
        if (!fs::exists(SESSION_DIR)) return 1;
        int maxId = 0;
        for (auto& entry : fs::directory_iterator(SESSION_DIR)) {
            string fname = entry.path().filename().string();
            // Parse "id:N.txt" format
            if (fname.substr(0, 3) == "id:" && fname.size() > 7) {
                string numStr = fname.substr(3, fname.size() - 7); // strip "id:" and ".txt"
                try { int id = stoi(numStr); if (id > maxId) maxId = id; }
                catch (...) {}
            }
        }
        return maxId + 1;
    ...
    void saveSessionLog() {
        ...
        ofstream f(filename);
        if (!f.is_open()) { printErr("Could not save session log"); return; }
...
public:
    ...
    void goBack() {
        if (backStack.isEmpty()) { printErr("Nothing to go back to"); return; }
    ...
    void goForward() {
        if (forwardStack.isEmpty()) { printErr("Nothing to go forward to"); return; }
    ...
    // =====================================================================
    void displayHistory() {
        cout << endl << C::BOLD << C::WHT << "  🕒 Full Browsing History ──" << C::R << endl;
        if (history.isEmpty()) { cout << C::DIM << "  (empty)" << C::R << endl; }
        else {
            vector<HistoryEntry> arr = history.toVector();
            for (int i = 0; i < (int)arr.size(); i++) {
                string star = bookmarks.contains(arr[i].url) ? C::YEL + " 🔖" + C::R : "";
                cout << C::DIM << "  " << setw(2) << (i + 1) << ". " << C::R
                     << C::CYN << arr[i].url << C::R << star
                     << C::DIM << "  @  " << arr[i].timestamp << C::R << endl;
            }
        }
        cout << C::DIM << "  Total entries: " << history.getCount() << C::R << endl;
        logAction("Displayed history (" + to_string(history.getCount()) + " entries)");
    }
    ...
    // =====================================================================
    void searchURL() {
        if (history.isEmpty()) { printErr("History is empty"); return; }
        string url;
        ...
        if (pos.empty()) {
            ...
            for (int p : pos) cout << C::GRN << "    #" << p << C::R << endl;
            logAction("Searched for \"" + url + "\" — found at " + to_string(pos.size()) + " position(s)");
        }
    }
    ...
    void deleteURL() {
        ...
        if (deleted.empty()) {
            ...
            for (auto& e : deleted) undoStack.push(e);
            printOK("Deleted " + to_string(deleted.size()) + " occurrence(s) of \"" + url + "\"");
            printInfo("You can undo up to " + to_string(undoStack.size()) + " deletion(s)");
            logAction("Deleted " + to_string(deleted.size()) + " occurrence(s) of \"" + url + "\"");
        }
    }
    ...
    // =====================================================================
    void undoDelete() {
        if (undoStack.isEmpty()) { printErr("Nothing to undo"); return; }
        ...
        cin >> levels;
        if (levels < 1 || levels > avail) { printErr("Invalid number"); return; }

        for (int i = 0; i < levels; i++) {
            HistoryEntry e = undoStack.pop();
            history.insertAtEnd(e);
            cout << C::GRN << "    Restored: " << e.url << " (" << e.timestamp << ")" << C::R << endl;
        }
    ...
    void bookmarkURL() {
        ...
        if (bookmarks.contains(url)) { printErr("Already bookmarked"); return; }
    ...
    void viewBookmarks() {
        ...
        if (bookmarks.isEmpty()) { cout << C::DIM << "  (empty)" << C::R << endl; }
        else {
        ...
        logAction("Viewed bookmarks (" + to_string(bookmarks.getCount()) + " total)");
    }

    void removeBookmark() {
        if (bookmarks.isEmpty()) { printErr("No bookmarks to remove"); return; }
        string url;
        ...
        if (del.empty()) { printErr("Bookmark not found"); logAction("Tried to remove bookmark \"" + url + "\" — not found"); }
        else { printOK("Removed bookmark: " + url); logAction("Removed bookmark: " + url); }
    }

    void mostVisited() {
        if (frequency.empty()) { printErr("No visit data available"); return; }

        // Copy to vector of pairs
        vector<pair<string, int>> freqVec(frequency.begin(), frequency.end());

        // Insertion sort descending by count (stable — preserves insertion order for ties)
        for (int i = 1; i < (int)freqVec.size(); i++) {
            auto key = freqVec[i];
            int j = i - 1;
            while (j >= 0 && freqVec[j].second < key.second) {
                freqVec[j + 1] = freqVec[j]; j--;
            }
            freqVec[j + 1] = key;
        }

        cout << endl << C::BOLD << C::WHT << "  📊 Top 5 Most Visited ──" << C::R << endl;
        int limit = min(5, (int)freqVec.size());
        for (int i = 0; i < limit; i++) {
            string star = bookmarks.contains(freqVec[i].first) ? C::YEL + " 🔖" + C::R : "";
            cout << C::GRN << "  " << setw(2) << (i + 1) << ". " << C::CYN << freqVec[i].first
                 << C::R << star << C::YEL << "  (" << freqVec[i].second << " visits)" << C::R << endl;
        }
    ...
    void sortHistory() {
        ...
        switch (ch) {
            case 1: insertionSortAZ(arr);
                    cout << endl << C::BOLD << "  ── Sorted A-Z ──" << C::R << endl;
                    break;
            case 2: insertionSortByCount(arr, frequency);
                    cout << endl << C::BOLD << "  ── Sorted by Visit Count ──" << C::R << endl;
                    break;
            case 3: insertionSortByRecent(arr);
                    cout << endl << C::BOLD << "  ── Sorted by Most Recent ──" << C::R << endl;
                    break;
            default: printErr("Invalid choice"); return;
        }
        ...
        for (int i = 0; i < (int)arr.size(); i++) {
            ...
            cout << C::DIM << "  " << setw(2) << (i+1) << ". " << C::R
                 << C::CYN << arr[i].url << C::R << star;
            if (ch == 2) cout << C::YEL << "  (" << frequency[toLower(arr[i].url)] << " visits)" << C::R;
    ...
    void saveToFile() {
        ...
        if (!f.is_open()) { printErr("Could not save session"); return; }

        f << "#CURRENT" << endl << currentPage << endl;

        f << "#HISTORY" << endl;
        vector<HistoryEntry> h = history.toVector();
        for (auto& e : h) f << e.url << "|" << e.timestamp << endl;

        f << "#BOOKMARKS" << endl;
        vector<HistoryEntry> b = bookmarks.toVector();
        for (auto& e : b) f << e.url << "|" << e.timestamp << endl;

        f << "#FREQUENCY" << endl;
        for (auto& p : frequency) f << p.first << "|" << p.second << endl;

        f << "#BACKSTACK" << endl;
        vector<string> bs = backStack.toVector();
        for (int i = (int)bs.size() - 1; i >= 0; i--) f << bs[i] << endl;

        f << "#FORWARDSTACK" << endl;
        vector<string> fs = forwardStack.toVector();
        for (int i = (int)fs.size() - 1; i >= 0; i--) f << fs[i] << endl;

        f << "#UNDOSTACK" << endl;
        vector<HistoryEntry> u = undoStack.toVector();
        for (int i = (int)u.size() - 1; i >= 0; i--) f << u[i].url << "|" << u[i].timestamp << endl;
    ...
    void loadFromFile() {
        ...
        if (!f.is_open()) return; // Fresh start
        ...
        while (getline(f, line)) {
            ...
            if (section == "#CURRENT") {
                ...
                if (sep != string::npos)
                    history.insertAtEnd(HistoryEntry(line.substr(0, sep), line.substr(sep + 1)));
            } else if (section == "#BOOKMARKS") {
                size_t sep = line.find('|');
                if (sep != string::npos)
                    bookmarks.insertAtEnd(HistoryEntry(line.substr(0, sep), line.substr(sep + 1)));
                ...
                size_t sep = line.find('|');
                if (sep != string::npos)
                    undoStack.push(HistoryEntry(line.substr(0, sep), line.substr(sep + 1)));
            }
        }
        f.close();
        if (!currentPage.empty())
    ...
    void run() {
        ...
        while (true) {
            ...
            printLine();
            switch (choice) {
                case 1:  visitURL(); break;
                case 2:  goBack(); break;
                case 3:  goForward(); break;
                case 4:  displayHistory(); break;
                case 5:  searchURL(); break;
                case 6:  deleteURL(); break;
                case 7:  undoDelete(); break;
                case 8:  bookmarkURL(); break;
                case 9:  viewBookmarks(); break;
                case 10: removeBookmark(); break;
                case 11: mostVisited(); break;
                case 12: sortHistory(); break;
                case 13:
                    ...
                    return;
                default: printErr("Invalid choice. Try 1-13.");
            }
            printLine();
        }
    }
